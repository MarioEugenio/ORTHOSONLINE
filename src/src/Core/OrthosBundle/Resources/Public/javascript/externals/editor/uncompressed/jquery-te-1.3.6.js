	...
	$.fn.jqte = function(options){
		...
		return this.each(function(){
			...
			if(!vars.status || !$(this).data("jqte"))
			{
				...
				{
					var lastValue = $(this).closest("."+vars.css).find("."+vars.css+"_editor").html();
					$(this).is("[value]") ? $(this).val(lastValue) : $(this).html(lastValue);
					var thisClone = $(this).closest("."+vars.css).find("textarea").clone();
					$(this).closest("."+vars.css).before(thisClone).remove();
					$(this).data("jqte",false);
				}
				return;
			}
			...
			{
				$this.prepend('<div class="'+vars.css+'_placeholder" unselectable><div class="'+vars.css+'_placeholder_text">'+vars.placeholder+'</div></div>');
				
				var placeHolder = $this.find("."+vars.css+"_placeholder");
			...
			function extractToText(strings)
			{
				...
				// no paragraph (<p>)
				if(!vars.p)
					$htmlContent = $htmlContent.replace(/\<p>(.*?)\<\/p>/ig, '<br/>$1');
				...
				// no paragraph and no break (<p> && <br>)
				if(!vars.p && !vars.br)
			...
			editor
				...
				.focus(function(){
					...
					// add onfocus class
					$this.addClass(vars.css+"_focused");
				})
				...
				.focusout(function(){
					...
					$this.removeClass(vars.css+"_focused");
				});
